...
module.exports = (path) => {
  ...
  if (isDocker()) {
    ...
    configuration.server.dist.config = `/root/dist/${configuration.server.dist.config}`;
    configuration.server.dist.web = `/root/dist/${configuration.server.dist.web}`;
  } else {
    configuration.global.dist = `${process.cwd()}/${configuration.global.dist}`;
